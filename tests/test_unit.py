...
"""
import pytest
from app.validators import validate_task_data, validate_priority, validate_title
...
class TestTaskModel:
    ...
    def test_task_dict_keys(self):
        ...
        task = Task(title='Task', task_id=1)
        data = task.to_dict()
        expected_keys = ['id', 'title', 'description', 'completed', 'priority', 'created_at', 'updated_at']
...
class TestUtils:
    ...
    def test_filter_by_priority_high(self):
        ...
        tasks = [
            Task(title='T1', priority='high'),
            Task(title='T2', priority='low')
        ]
