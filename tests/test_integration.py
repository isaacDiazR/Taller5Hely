...
class TestGetTasksEndpoint:
    ...
    def test_get_tasks_filter_completed(self, client):
        ...
        response = client.post('/api/tasks', data=json.dumps({'title': 'Task 1'}), content_type='application/json')
        task_id = json.loads(response.data)['task']['id']
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'completed': True}), content_type='application/json')
        
        # Crear otra tarea sin completar
        client.post('/api/tasks', data=json.dumps({'title': 'Task 2'}), content_type='application/json')
    ...
    def test_get_tasks_filter_by_priority(self, client):
        ...
        client.post('/api/tasks', data=json.dumps({'title': 'T1', 'priority': 'high'}), content_type='application/json')
        client.post('/api/tasks', data=json.dumps({'title': 'T2', 'priority': 'low'}), content_type='application/json')
...
class TestGetTaskByIdEndpoint:
    ...
    def test_get_task_by_id_success(self, client, sample_task_data):
        ...
        # Crear tarea
        create_response = client.post('/api/tasks', data=json.dumps(sample_task_data), content_type='application/json')
...
class TestUpdateTaskEndpoint:
    ...
    def test_update_task_mark_completed(self, client):
        ...
        # Crear tarea
        create_response = client.post('/api/tasks', data=json.dumps({'title': 'Task'}), content_type='application/json')
        task_id = json.loads(create_response.data)['task']['id']
...
class TestStatsEndpoint:
    ...
    def test_stats_with_tasks(self, client):
        ...
        # Crear tareas
        response1 = client.post('/api/tasks', data=json.dumps({'title': 'T1', 'priority': 'high'}), content_type='application/json')
...
class TestUserFlows:
    ...
    def test_create_and_complete_task_flow(self, client):
        ...
        # Crear
        response = client.post('/api/tasks', data=json.dumps({'title': 'My Task'}), content_type='application/json')
        assert response.status_code == 201
        task_id = json.loads(response.data)['task']['id']
        
        # Completar
        response = client.put(f'/api/tasks/{task_id}', data=json.dumps({'completed': True}), content_type='application/json')
        assert response.status_code == 200
    ...
    def test_create_update_delete_flow(self, client):
        ...
        # Crear
        response = client.post('/api/tasks', data=json.dumps({'title': 'Original'}), content_type='application/json')
        task_id = json.loads(response.data)['task']['id']
        
        # Actualizar
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'title': 'Updated'}), content_type='application/json')
    ...
    def test_multiple_tasks_management(self, client, multiple_tasks_data):
        ...
        for i in range(0, len(task_ids), 2):
            client.put(f'/api/tasks/{task_ids[i]}', data=json.dumps({'completed': True}), content_type='application/json')
    ...
    def test_priority_workflow(self, client):
        ...
        for priority in priorities:
            client.post('/api/tasks', data=json.dumps({'title': f'Task {priority}', 'priority': priority}), content_type='application/json')
    ...
    def test_bulk_operations(self, client):
        ...
        task_ids = []
        for i in range(10):
            response = client.post('/api/tasks', data=json.dumps({'title': f'Task {i}'}), content_type='application/json')
            task_ids.append(json.loads(response.data)['task']['id'])
    ...
    def test_update_priority_flow(self, client):
        ...
        response = client.post('/api/tasks', data=json.dumps({'title': 'Task', 'priority': 'low'}), content_type='application/json')
        task_id = json.loads(response.data)['task']['id']
        
        # Escalar a media
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'priority': 'medium'}), content_type='application/json')
        response = client.get(f'/api/tasks/{task_id}')
        assert json.loads(response.data)['task']['priority'] == 'medium'
        
        # Escalar a alta
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'priority': 'high'}), content_type='application/json')
        response = client.get(f'/api/tasks/{task_id}')
    ...
    def test_error_recovery_flow(self, client):
        ...
        response = client.post('/api/tasks', data=json.dumps({}), content_type='application/json')
        assert response.status_code == 400
        
        # Crear tarea válida después del error
        response = client.post('/api/tasks', data=json.dumps({'title': 'Valid Task'}), content_type='application/json')
        assert response.status_code == 201
    ...
    def test_concurrent_operations(self, client):
        ...
        # Crear tarea
        response = client.post('/api/tasks', data=json.dumps({'title': 'Shared Task'}), content_type='application/json')
        task_id = json.loads(response.data)['task']['id']
        
        # Múltiples actualizaciones consecutivas
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'description': 'Update 1'}), content_type='application/json')
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'description': 'Update 2'}), content_type='application/json')
        client.put(f'/api/tasks/{task_id}', data=json.dumps({'description': 'Final Update'}), content_type='application/json')
